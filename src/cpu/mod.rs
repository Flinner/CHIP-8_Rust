...
pub struct CPU {
    ...
    pub stack_p: usize,
    pub delay_timer: u8,
    /// beeps as long as it is above 0
    pub sound_timer: u8,
    pub reg: [u8; 16],
    // pub display: Display,
}
...
impl CPU {
    ...
    pub fn exectue(
        ...
        match (n1, x, y, n) {
            ...
            (0x8, _, _, 0x5)     => opcodes::sub_vx_vy       			(self, x, y),
            (0x8, _, _, 0x6)     => opcodes::shift_vy_right       		(self, x, y),
            (0x8, _, _, 0x7)     => opcodes::sub_vy_vx       			(self, x, y),
            (0x8, _, _, 0xE)     => opcodes::shift_vy_left       		(self, x, y),
            (0x9, _, _, 0x0)     => opcodes::skip_if_vx_neq_vy       		(self, x, y),
            (0xA, _, _, _)       => opcodes::set_index_reg_nnn       		(self, nnn),
            (0xB, _, _, _)       => opcodes::jump_with_offset       		(self, nnn, x, nn),
            (0xC, _, _, _)       => opcodes::random_to_vx       		(self ,x,nn),
            (0xD, _, _, _)       => opcodes::draw       			(self, x, y, n),
            (0xE, _, 0x9, 0xE)   => opcodes::skip_if_vx_eq_key_pressed       	(self, x),
            (0xE, _, 0xA, 0x1)   => opcodes::skip_if_vx_eq_key_not_pressed      (self, x),
            ...
            (0xF, _, 0x1, 0xE)   => opcodes::add_vx_to_index                    (self, x),
